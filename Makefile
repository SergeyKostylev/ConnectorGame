...
# Generate a v2 level: saves json + png to levels/
# Params: rows, cols, batteries=3, run=1 (0 за замовчуванням)
# make generate-level-v2
# make generate-level-v2 rows=4 cols=6
# make generate-level-v2 rows=5 cols=5 batteries=3
# make generate-level-v2 run=1
generate-level-v2:
	python generate.py v2 $(if $(rows),$(rows),) $(if $(cols),$(cols),) $(if $(batteries),batteries=$(batteries),) $(if $(run),run,)

# Generate a v3 level: like v2 but with controlled targets_percent (лампочки)
# Params: rows, cols, batteries=3, targets_percent=15, run=1 (0 за замовчуванням)
